...
def bisect_root(func, a, b, tol=1e-12, max_iter=200):
    ...
    for _ in range(max_iter):
        ...
        if not np.isfinite(fc):
            # small numerical shift if needed
            c = np.nextafter(c, b)
            fc = func(c)
...
def solve_modes_TE(lmbd, n_core, n_clad, h, eps=1e-12, m_max=10, Nscan=4000):
    ...
    # Remove duplicates (if two brackets gave the same root)
    modes = _deduplicate_modes(modes, tol=1e-8)

    # Sort by decreasing neff and renumber modes
...
def build_field_profile(mode, lmbd, n_core, n_clad, h, x_grid):
    ...
    if mode["parity"] == "even":
        ...
        field[clad_mask] = (
            E_boundary * np.sign(x[clad_mask]) *
            np.exp(-g_val * (np.abs(x[clad_mask]) - a))
        )
...
def extract_branches(results, lambda_list):
    ...
    branches = {
        m: {
            "lambda": lambda_list.copy(),
            "neff": np.full(len(lambda_list), np.nan),
            "ng":   np.full(len(lambda_list), np.nan)
        } for m in all_ms
    }
...
if __name__ == "__main__":
    n_core = float(input("Enter n_core: "))
    n_clad = float(input("Enter n_cladding: "))
    h      = float(input("Enter slab thickness h (same units as λ): "))

    lam_min = float(input("Enter lambda_min: "))
    lam_max = float(input("Enter lambda_max: "))
    Nlam    = int(input("Enter number of wavelength points (>=3): "))
    ...
    target = float(input("For field profile: enter lambda0 (e.g., 1.55): "))
